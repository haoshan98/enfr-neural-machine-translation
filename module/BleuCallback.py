...
from module.model0 import bleuInferenceModel2
...
class BleuCallback(keras.callbacks.Callback):
    def __init__(self, en_input_sequences, de_input_sequences, 
                 v_en_input_sequences, v_de_input_sequences, 
                 en_trigger_sequences, de_trigger_input_sequences, bsize, en_tok, de_tok,
                 logging, en_inputs, en_state, 
        ...
        'Initialization'
        self.en_x_ = en_input_sequences
        self.de_x_ = de_input_sequences
...
#                                    self.tr_de_dense_2, self.HID_DIM)
            ...
            if self.getFast:
    #             train_score = self.cal_bleu_score("Train", epoch, self.en_x_, self.de_x_, inf_encoder,
    #                                           inf_decoder)
                valid_score = self.cal_bleu_score_fast("Valid", epoch, self.v_en_x_, self.v_de_x_,
                                              inf_encoder, inf_decoder)
            else:
                valid_score = self.cal_bleu_score("Valid", epoch, self.v_en_x_, self.v_de_x_,
                                              inf_encoder, inf_decoder)
            # bleu for trigger set
            source_sent, source_sents = get_source_sents(self.t_en_x, self.en_tok,
                                                         max_len=MAX_LENGTH,
                                                         size=self.bsize) 
            hypo_sent, hypo = get_hypo_sents(self.t_en_x, inf_encoder, inf_decoder, 
                                             self.de_tok, max_len=MAX_LENGTH,
                                             GRU_LAYER=self.GRU_LAYER)
